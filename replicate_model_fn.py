  ...
  def split_dictionary(dictionary):
    ...
    for name, tensor in six.iteritems(dictionary):
      if isinstance(tensor, sparse_tensor.SparseTensor):
        ...
        _tensor_shards = [array_ops.split(_, num_or_size_splits=number_of_shards) for _ in tensor]
        for i, shard in enumerate(zip(*_tensor_shards)):
          shards[i][name] = shard
      else:
  ...
  with ops_lib.name_scope('split_inputs'):
    with ops_lib.device(device):
      ...
      if labels is None:
        ...
        _label_shards = [array_ops.split(_, num_or_size_splits=number_of_shards) for _ in labels]
        label_shards = [_ for _ in zip(*_label_shards)]
      else:
